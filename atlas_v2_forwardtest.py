"""
ATLAS V2: Forward Testing Module
===============================
Simulates paper/live walkforward on OOS data or streaming updates.
"""
...
logger = logging.getLogger("atlas_v2_forwardtest")
...
class ForwardTester:
    def __init__(
        ...
        config,
        data_provider,
        result_dir='forwardtest_logs',
        name: Optional[str] = None,
    ):
        self.agent = agent
        self.config = config
        self.market = data_provider  # Can be a rolling or streaming class
        self.name = name or f'forwardtest_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    def run(self, steps=None):
        logger.info(f"Running forward test: {self.name}")
        steps = steps or len(self.market)
        for step in range(steps):
            # Simulate streaming by only "revealing" up to current index
    ...
    # The following methods closely mirror the Backtester, adjusted for OOS/stream context
        ...
        for symbol in self.config.symbols:
            ...
            if signal != 0:
                if (signal == 1 and position < 0) or (signal == -1 and position > 0):
                    gained = (price - entry_price) * abs(self.portfolio[symbol]['size']) * np.sign(position)
                    self.balance += gained
                    self.portfolio[symbol]['pnl'] += gained
                    self.portfolio[symbol]['position'] = 0
                    self.portfolio[symbol]['size'] = 0
                    trade = dict(
                        symbol=symbol, action="LIQUIDATE", price=price, pnl=gained,
                        size=self.portfolio[symbol]['size'], step=self.step,
                ...
                trade = dict(
                    ...
                    size=size, step=self.step, time=datetime.now().isoformat(),
                )
    ...
    def _final_report(self):
        logger.info(f"==== FORWARD TEST COMPLETE ({self.name}) ====")
    ...
    def _persist_logs(self):
        ...
        logger.info(f"Forward test logs written to {self.result_dir}")
